...
**[&#x1F7E2;]** The PR meets requirements. | **[&#x1F534;]** The PR does not meet requirements. | **[&#x25EF;]** The PR is not applicable. | **[&#x1F7E1;]** The PR is questionable. | **[&#x1F535;]** The PR is under review.
**NOTE:** Comment "/review status[go/nogo/na/question/ongoing]:number_list[0,1,2 ...] ..." to update the status.
Example: "/review go:0,1,2 nogo:3,4,5" or "/review go:0-2 nogo:3-5".
Comment "/review status[go/nogo/na/question/ongoing]:999" if you want to update the status of all items at a time.
...
CHECKLIST_EN = "helper/reviewer_checklist_en.yaml"
...
categorizer = {'PRSubmissionSPEC': 'PR提交规范',
               ...
               'customization': '定制项'}
categorizer_en = {'PRSubmissionSPEC': 'PR Submission Specifications',
               'CleanCode': 'Clean Code',
...
def check_repository_changes():
    """
    check if any repository yaml has been changed
    """
    current_branch = get_current_branch()
    fetch_branch = current_branch.split('working_')[-1]
...
def parse_sig_info_members(info):
    ...
    if isinstance(maintainers, list):
        for maintainer in maintainers:
            member = maintainer.get('gitee_id')
    ...
    for repo in repos:
        ...
        if isinstance(committers, list):
            for committer in committers:
                member = committer.get('gitee_id')
                if member not in members:
                    members.append(member)
        repo_admin = repo.get('repo_admin')
        if isinstance(repo_admin, list):
            for admin in repo_admin:
                member = admin.get('gitee_id')
                if member not in members:
                    members.append(member)
    return members


def check_members_add(info):
    """
    check if new members(maintainer, committers or repo_admin) add in sig-info.yaml
    """
    review_body = ""
    diff_files = subprocess.getoutput('git diff --name-status remotes/origin/master..')
    new_members = []
    for line in diff_files.splitlines():
        status, item = line.split(maxsplit=1)
        item_slices = item.split('/')
        if len(item_slices) != 3:
            continue
        if item_slices[0] == 'sig' and item_slices[2] == 'sig-info.yaml':
            if status == 'A':
                sig_info_now = yaml.safe_load(open(item).read())
                ...
                sig_info_now = yaml.safe_load(open(item).read())
                sig_info_before = yaml.safe_load(subprocess.getoutput('git show remotes/origin/master:{}'.format(item)))
                members_now = parse_sig_info_members(sig_info_now)
                ...
                for member in members_now:
                    ...
                    if member not in new_members:
                        new_members.append(member)
    if new_members:
        review_body += join_check_item(categorizer['customization'], info['claim'], info['explain'].format(
            new_members=' '.join('@' + member for member in new_members)))
    return review_body


def check_new_members_confirmation(user_gitee, owner, repo, number, review_comment):
    """
    Check whether new members added in the PR, if match then add label `wait_confirm`
    """
    if '等所有人' in review_comment or 'approved by all members' in review_comment:
        pr_labels = user_gitee.get_pr_labels_all(owner, repo, number)
        if 'wait_confirm' not in [pr_label['name'] for pr_label in pr_labels]:
            user_gitee.add_pr_labels(owner, repo, number, ["wait_confirm"])
...
def check_maintainer_changes():
    ...
    for line in lst_files.splitlines():
        ...
        elif item.startswith("sig/") and item.endswith("/sig-info.yaml"):
            sig = item.split("/")[1]
            if os.path.exists('sig/{}/OWNERS'.format(sig)):
                continue
            sig_info = yaml.load(open(item, 'r'), Loader=yaml.Loader)
            maintainers = sig_info['maintainers']
            current_maintainers = []
            if maintainers:
                current_maintainers = [x['gitee_id'] for x in maintainers]
            remote_sig_info = subprocess.getoutput('git show remotes/origin/master:' + item)
            if remote_sig_info.startswith('fatal'):
                remote_owners = subprocess.getoutput('git show remotes/origin/master:' +
...
def community_review(custom_items):
    ...
    for cstm_item in custom_items:
        ...
        elif cstm_item['condition'] == 'new-members-add':
            review_body += check_members_add(cstm_item)
...
def remove_useless_review_comment(user_gitee, group, repo_name, comment_id):
    """
    Remove useless review comment
    """
    user_gitee.delete_pr_comment(group, repo_name, comment_id)
...
def find_review_comment(user_gitee, group, repo_name, pull_id):
    ...
    all_review_comments_id = []
    for comment in data:
        if review_key in comment['body'] or review_key_en in comment['body']:
            if not latest_review_comment:
                latest_review_comment = comment
                all_review_comments_id.append(comment['id'])
            else:
                if comment['id'] > latest_review_comment['id']:
                    latest_review_comment = comment
                    all_review_comments_id.append(comment['id'])
    for review_comment_id in all_review_comments_id[:-1]:
        remove_useless_review_comment(user_gitee, group, repo_name, review_comment_id)
    return latest_review_comment
...
def choose_language(pull_request):
    try:
        title = pull_request['title']
        body = pull_request['body']
    except KeyError:
        return
    if not is_contains_chinese(title) and not is_contains_chinese(body):
        global CHK_TABLE_HEADER
        global CHECKLIST
        global categorizer
        CHK_TABLE_HEADER = CHK_TABLE_HEADER_EN
        CHECKLIST = CHECKLIST_EN
        categorizer = categorizer_en
...
def main():
    ...
    if args.edit:
        ...
        find_review_comment(user_gitee, group, repo_name, pull_id)
        check_new_members_confirmation(user_gitee, group, repo_name, pull_id, review_comment)
        if args.clean:
